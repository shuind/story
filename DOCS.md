...
- **卡片操作**：墨签上直接提供「写」「连」「×」；点击「写」展开笔记，不依赖双击或隐藏悬停按钮。
  - `fold 0` 墨签：保留画布留白，同时提供最常用操作。
  - `fold 1` 半展：显示摘要、节点类型和「我的思考」输入区。
  - 阅读：卡片操作条或 Library 的「读」→ 打开右侧抽屉。
- **选中与投影**：单击卡片选中（Shift 累加）→ 底部浮出投影条，显示已选数量。
- **浮动工具条**：单选时可以写笔记、阅读、从此处连线；多选时可以输入关系、连接、复制 Prompt 或批量删除。
...
- **连线**：选中两张卡片后直接在工具条输入关系并连接；或者点一张卡片的「连」后再点另一张卡片的「连」。双击关系线可以移除。
