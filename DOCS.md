...
> 当前版本已经打通本地素材库、画布持久化、导入导出、素材编辑保存、多画布管理、Path 和 SSH GitHub commit/push。多模型执行和跨设备同步仍属于后续运行时能力。
...
| **Card（卡片）** | Plugin 或 Element 被放到画布上的一个实例。同一项目可放多张。 | 画布上的一块墨签或维度卡 |
...
components/
  ...
  plugin-drawer.tsx     右侧抽屉：阅读维度说明与所含元素
  quick-add-panel.tsx   Ctrl/⌘ K 快速添加维度或元素
  input-dialog.tsx      新建、重命名、Path 操作的统一表单弹窗
...
- **放置元素/维度**：左栏点「放」添加到画布；也可以直接把维度或元素拖到画布的指定位置；按 `Ctrl/⌘ K` 可搜索并快速添加。快速添加面板按文件夹层级展示 Plugin，展开后选择其中的 Element；搜索会自动展开命中的文件夹。`←/→` 可收起/展开文件夹。
- **画布平移**：按住 `Space` 后拖动，或使用鼠标中键拖动；空白左键拖动用于框选。
- **框选与多选**：空白处拖出选择框；按住 `Shift` / `Ctrl` / `⌘` 点击可累加或移除选择。
- **缩放**：滚轮以指针为中心；顶栏可缩小、放大、重置和「适配」全部卡片。快捷键 `+` / `-` / `0` 同样可用。
- **撤销与重做**：顶栏提供 `↶ / ↷`；`Ctrl/⌘ Z` 撤销，`Ctrl/⌘ Shift Z` 或 `Ctrl/⌘ Y` 重做。会记录卡片、关系线和视口变化；输入框内仍保留原生文字撤销。
- **卡片操作**：墨签只显示标题，保持画面干净；展开卡片只保留内容、全文和移除，连线统一从底部工具条进入。
  - `fold 0` 墨签：保留画布留白，不塞入操作控件。
  - `fold 1` 半展：显示摘要、节点类型和「我的思考」输入区。
  - 阅读：展开卡片的「全文」或 Library 的「读」→ 打开右侧抽屉。
- **选中与投影**：单击卡片选中（Shift 累加）→ 底部浮出投影条，显示已选数量。
- **浮动工具条**：单选时可以切换写笔记、阅读、从此处连线；多选时可以输入关系、连接、复制 Prompt 或批量删除。所有模式都有明确的取消入口。
- **键盘操作**：`Ctrl/⌘ A` 全选，`Ctrl/⌘ D` 复制选中卡片，方向键移动（`Shift` 加速），`Delete` / `Backspace` 删除，`Escape` 退出当前模式。
- **复制粘贴**：`Ctrl/⌘ C` 暂存选中卡片，`Ctrl/⌘ V` 在画布中偏移粘贴；会保留选中卡片之间的关系线。
- **维度投影**：选中 Plugin 卡片时，会把该维度下的全部 Element 正文纳入 Prompt；与单个 Element 重复时自动去重。
- **阅读维度**：Library 的 Plugin 行和画布 Plugin 卡片都有「读」入口，可查看维度说明与所含元素。
- **画布思考**：卡片展开后可在「我的思考」里写判断、问题、冲突或下一步；内容随画布保存。
- **删除卡片**：卡片上的「×」、底部「删除选中」，或键盘 `Delete` / `Backspace` 都可以删除选中卡片，相关连线会一起移除。
- **连线**：选中两张卡片后直接在工具条输入关系并连接；也可以单选后进入「从这里连线」，再点击目标卡片。双击关系线可以移除。
...
**现状**：`app/page.tsx` 会把每个画布的状态（卡片、思考笔记、关系线与视图）写入 `localStorage`，画布索引使用 `story-canvas:index`，单个画布使用 `story-canvas:<id>`。
